...
# from application import api          # api section is not working because of library dependencies.
...
###### something is wrong with the flask_restplus api ######
# @api.route('/api', '/api/')
...
def enrollment():
    ...
    if courseID:
        if Enrollment.objects(user_id = user_id, courseID = courseID):
            flash(f"You are already registered in the course {courseTitle}!!!", "danger")
            return redirect(url_for("courses"))
        else:
            enrollment = Enrollment(user_id=user_id, courseID = courseID)
            flash(f"You are enrolled in course {courseTitle} !", "success")
            enrollment.save()
